...
bool enable_bloom{ false };  // always hdr, so just control bloom compare effect
float exposure{ 1.0f };
...
void RenderLoop() {
	...
	unsigned int container_texture{ LoadTexture("container2.png", true) };
	Shader shader{ "bloom.vert", "bloom.frag" };
	Shader light_shader{ "bloom.vert", "light_box.frag" };
	Shader blur_shader{ "blur.vert", "blur.frag" };
	Shader hdr_bloom_final_shader{ "bloom_final.vert", "bloom_final.frag" };

	shader.use();
	shader.setInt("diffuseTexture", 0);
	blur_shader.use();
	blur_shader.setInt("image", 0);
	hdr_bloom_final_shader.use();
	hdr_bloom_final_shader.setInt("hdrBuffer_scene", 0);
	hdr_bloom_final_shader.setInt("bloomBlur", 1);
	...
	light_positions.emplace_back(glm::vec3{ 0.0f, 0.5f,  1.5f });
	light_positions.emplace_back(glm::vec3{ -4.0f, 0.5f, -3.0f });
	light_positions.emplace_back(glm::vec3{ 3.0f, 0.5f,  1.0f });
	light_positions.emplace_back(glm::vec3{ -0.8f,  2.4f, -1.0f });
	light_colors.emplace_back(glm::vec3{ 5.0f,   5.0f,  5.0f });
	light_colors.emplace_back(glm::vec3{ 10.0f, 0.0f, 0.0f });
	light_colors.emplace_back(glm::vec3{ 0.0f, 0.0f, 15.0f });
	light_colors.emplace_back(glm::vec3{ 0.0f, 5.0f, 0.0f });
	...
	// expilitly tell OpenGL which color attachment we'll use(of this framebuffer) for rendering
	unsigned int attachments[2] { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 };
	glDrawBuffers(2, attachments);  // we could set layout location 0, 1 in fragment shader
	...
	for (int i = 0; i < 2; ++i) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, kScreenWidth, kScreenHeight, 0, GL_RGBA, GL_FLOAT, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // we clamp to the edge as the blur filter would otherwise sample repeated texture values!
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, pingpongColorbuffers[i], 0);
		// here blur a 2D texture, so we don't need depth renderring buffer
		if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (unsigned int i = 0; i < light_positions.size(); i++) {
			...
			light_shader.setVec3("lightColor", light_colors[i]);
			vertex.Draw(vertex.cubeVAO);
		}
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		...
		for (int i = 0; i < blur_amount; ++i) {
			glBindFramebuffer(GL_FRAMEBUFFER, pingpongFBO[static_cast<int>(horizontal)]);
			blur_shader.setBool("horizontal", horizontal);
			glBindTexture(GL_TEXTURE_2D, first_iteration ? colorBuffers[1] : pingpongColorbuffers[static_cast<int>(!horizontal)]);
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		hdr_bloom_final_shader.use();
		hdr_bloom_final_shader.setBool("enable_bloom", enable_bloom);
		hdr_bloom_final_shader.setFloat("exposure", exposure);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, colorBuffers[0]);
		glActiveTexture(GL_TEXTURE1);
		glBindTexture(GL_TEXTURE_2D, pingpongColorbuffers[static_cast<int>(!horizontal)]);
		glBindVertexArray(vertex.quadVAO);
		vertex.Draw(vertex.quadVAO);
...
int InitWindow() {
	...
	window = glfwCreateWindow(kScreenWidth, kScreenHeight, "Advanced Lighting: Bloom", nullptr, nullptr);
	if (!window) {
